...
* an ssh public/private key pair (see the good `Github docs on generating ssh keys
  ...
  * register in the Gerrit server. See `Registering your SSH key with Gerrit`_
...
merge, vote +1. If there isn't an automated job to test your change and vote +1/-1
...
Once the change gets published and you've voted for merging, add the people who
...
1. Generate your GPG key.
   ...
   If you're coolaborating in keysigning, then send the output of
...
#. Edit your ``.bash_profile`` file to contain the following:
