...
It is always a good habit to do as much checks we can locally before we submit it.
...
If all is ok, it will not return anything, while if you have an issue it will return an error.

Here is an INFO file with multiple name rows under the type (only allowed one name row).
