...
of files that store configuration or credentials. These files managed out of
the ci-management repository along side of the rest of the community
configurable components under the ``jenkins-admin/managed-config-files``
directory tree.
...
The content file is the actual file that is under management. This must be an
...
The mapping type will used verbatim when converting to the JCasC so it should
be properly configured to match the config_type.
