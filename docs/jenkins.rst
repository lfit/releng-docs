...
scheduled intervals. On the production environment any logs older than
6 months gets purged everyday (at 08:00 UTC) and on the sandbox all
logs are purged on a weekly basis (every Satruday at 08:00 UTC).

Jenkins sandbox is also setup with cron jobs to delete all jobs on a weekly
basis (every Satruday at 08:00 UTC).
