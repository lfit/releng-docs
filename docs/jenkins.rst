...
Documentation is available for `installing virtual environment <https://virtualenvwrapper.readthedocs.org/en/latest/>`_.
On Linux systems with pip (typical), they amount to:
...
#. Install JJB
   ...
   .. note::
       ...
       If a requirements.txt exists in the repo with the recommended JJB version.
       It can be used to install JJB by calling. ``pip install -r jjb/requirements.txt``.
