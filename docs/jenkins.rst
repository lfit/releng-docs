...
Jenkins Job Builder is used to translate YAML code to job configuration suitable
...
JJB can be installed with `pip <Installing JJB using pip_>`_  or directly from
...
as a workaround for any known issues with the latest version. The documentation is available in
...
dependencies, make sure you've `created and activated <Virtual Environments_>`_ a virtual environment for JJB.
