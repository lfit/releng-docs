...
1. Generate your GPG key.
  ...
  b. Enter a minimum of 4096 bit key size.
  c. Enter an expiration time, picked 2y for 2 years
...
.. note::
    ...
    The key ring default path `.gnugp` on Linux is `/home/$USER/.gnupg/` and
    MacOS is `/Users/$USER/.gnupg/`. However this setting can be overridden by
    setting the environment variable $GNUPGHOME.

2. Get the fingerprint for the keys created.
...
.. code:: bash
   ...
   gpg2 --send-keys <KEY-FINGERPRINT>
   ...
   gpg: sending key <KEY-FINGERPRINT> to hkp server keys.gnupg.net
