...
To browse artifacts, users do not need to log in but rather browse as an annonymous
user.
...
Nexus 3 does not require to define patterns for Repository Targets to only allow a
specific directory structure to be created.

Just like Nexus 2, we require to have one user entry per repo in Nexus 3.

The following parameters need to be provided for every user:
...
:Status: Active. Can be set to Disabled if the Gerrit repo is no longer in use

:Roles: docker. This role will allow the user to administrate docker images.
